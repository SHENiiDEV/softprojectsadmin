...
class CredentialVault extends Component
{
    ...
    public function updatedFilterProject(): void  { $this->loadData(); }
    ...
    protected function loadData(): void
    {
        ...
        // Curators and admins see all; managers/others see only their projects
        if ($user && !$user->hasAnyRole(['admin', 'curator'])) {
        ...
        // Sidebar data
        if ($user && !$user->hasAnyRole(['admin', 'curator'])) {
            $this->projects = Project::where('manager_id', $user->id)->orderBy('name')->get(['id', 'name', 'status']);
        } else {
